...
<details>
  <summary>Версия от 27.04.2026</summary>

### OpenVPN-Status
1. Добавлена переменная WORKER_COUNT (Кол-во ядер * 2 = WORKER_COUNT) для запуска [Gunicoin](https://gunicorn.org/reference/settings/#workers).
...
### Telegram-Bot
1. Добавлен API Amnezia Wireguard. Создание/удаление/список клиентов итд, происходит через API

### OpenVPN-Status
1. Добавлен суточный график.
2. На главную страницу добавляются сетевые интерфейсы контейнеров AmnesiaWG и OpenVPN, если они существуют. Отображаются в графике скорости и в статистике накопленного трафика.
3. Добавлено отображение накопленного трафика интерфейса с момента Uptime сервера и текущего календарного месяца.
</details>
