...
def create_clientmap_delete_menu(telegram_id: str, client_name: str):
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Удалить", callback_data=f"clientmap_delete_confirm_{telegram_id}_{client_name}"),
...
def create_client_list_keyboard(clients, page, total_pages, vpn_type, action):
    """Создает постраничную клавиатуру со списком клиентов с датами истечения."""
    buttons = []
    ...
    for client in clients[start_idx:end_idx]:
        # Парсим имя и дату (формат: "Имя|ДД-ММ-ГГГГ")
        if isinstance(client, dict):
            client_name = client.get("name", str(client))
            expire_date = client.get("expire")
        elif "|" in str(client):
            parts = str(client).split("|", 1)
            client_name = parts[0].strip()
            expire_date = parts[1].strip() if len(parts) > 1 else None
        else:
...
def create_user_profile_selection_keyboard(profiles: list):
    """Создать клавиатуру выбора профиля для пользователя."""
    buttons = []
...
def create_user_info_keyboard():
    """Создать клавиатуру «Назад к профилям»."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
...
def create_forward_message_keyboard():
    """Создать клавиатуру для пересылки сообщения."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
...
def create_profile_mapping_keyboard(clients: list):
    """Создать клавиатуру выбора профиля для привязки к пользователю."""
    buttons = []
...
def create_no_profiles_keyboard():
    """Создать клавиатуру когда нет профилей."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="clients_menu")]
        ]
