...
async def handle_callback_query(callback: types.CallbackQuery, state: FSMContext):
    ...
    data = callback.data
    logger.debug(f"🔘 Callback от {callback.from_user.id}: {data}")
    ...
    try:
        ...
        if data.startswith("req_no_"):
            ...
            if uid.isdigit():
                bot = await _get_bot()
                try:
                    await bot.send_message(int(uid), "❌ Ваш запрос доступа отклонён администратором.")
                except Exception:
                    pass
                ...
                logger.info("📬 Запрос доступа отклонён для пользователя %s", uid)
        ...
        if data.startswith("req_bind_"):
            ...
            if uid.isdigit() and client_name:
                ...
                logger.info("✅ Запрос доступа одобрен: %s → %s", uid, client_name)
            await callback.answer()
            return
        ...
        logger.error(f"❌ Error в callback handler: {e}")
...
async def handle_request_client_name_input(message: types.Message, state: FSMContext):
    ...
    logger.info("✅ Запрос доступа одобрен через ввод имени: %s → %s", uid_str, client_name)