...
async def handle_server_stats(callback: types.CallbackQuery):
    ...
    if callback.from_user.id not in admin_ids:
        logger.warning("❌ Отказано в доступе к статистике для %s", callback.from_user.id)
        await callback.answer("Доступ запрещен!", show_alert=True)
        return
    
    stats_text = await get_server_stats()
    logger.debug("📊 Статистика запрошена админом %s", callback.from_user.id)
...
async def handle_server_reboot(callback: types.CallbackQuery):
    ...
    if callback.from_user.id not in admin_ids:
        await callback.answer("Доступ запрещен!", show_alert=True)
        return
    logger.info("🔄 Запрос перезагрузки от админа %s", callback.from_user.id)
...
async def handle_server_reboot_confirm(callback: types.CallbackQuery):
    ...
    logger.critical("🔄 Перезагрузка сервера подтверждена админом %s", callback.from_user.id)  # CRITICAL - критическое действие
...
async def handle_memory_threshold_input(message: types.Message, state: FSMContext):
    ...
    logger.info("⚙️ Порог RAM обновлён до %s%% админом %s", value, message.from_user.id)  # INFO - важное действие
