...
from .common import get_client_info_for_user
...
from ..utils import execute_script, get_clients, cleanup_openvpn_files, get_external_ip, format_days
...
async def handle_client_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.from_user.id not in admin_ids:
        ...
        if not client_name or client_name not in allowed_clients:
            logger.warning("❌ Отказано в доступе пользователю %s", callback.from_user.id)
...
async def handle_skip_expire(callback: types.CallbackQuery, state: FSMContext):
    admin_ids = get_admin_ids()
    if callback.from_user.id not in admin_ids:
        logger.warning("❌ Несанкционированный доступ к skip_expire от %s", callback.from_user.id)
        await callback.answer("Доступ запрещен!", show_alert=True)
        return
    
    current_state = await state.get_state()
    if current_state != VPNSetup.entering_days:
        logger.warning("❌ Сессия истекла для пользователя %s", callback.from_user.id)
    ...
    if not client_name or option != "1":
        logger.error("❌ Ошибка: данные клиента не найдены для %s", callback.from_user.id)
        await callback.answer("❌ Ошибка: данные клиента не найдены", show_alert=True)
        await state.clear()
        return

    await callback.message.delete()
    logger.info("🔧 Создание клиента %s со сроком действия сертификата (1825 дней)", client_name)
    result = await execute_script("1", client_name, "1825")

    if result["returncode"] == 0:
        await send_config(callback.from_user.id, client_name, "1")
        server_ip = _get_server_ip()
        await callback.message.answer(f"✅ Клиент создан!\n📅 Срок действия: 1825 дней (5 лет)", reply_markup=create_main_menu(server_ip))
        log_action("bot", callback.from_user.id, callback.from_user.full_name, "client_create", f"{client_name} (openvpn)")
        await notify_admins(callback.from_user.id, callback.from_user.full_name, f"создал клиента <b>{client_name}</b> (openvpn)")
        logger.info("✅ Клиент %s создан успешно", client_name)
    else:
...
async def handle_protocol_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data.startswith("send_ovpn_"):
        ...
        if proto == "default":
            dir_path = f"/root/web/openvpn/client/"
...
async def handle_client_name(message: types.Message, state: FSMContext):
    ...
    if not re.match(r"^[a-zA-Z0-9_.-]{1,32}$", client_name):
        logger.warning("❌ Некорректное имя клиента от %s", message.from_user.id)
...
@router.callback_query(lambda c: c.data == "skip_expire")
async def handle_skip_expire(callback: types.CallbackQuery, state: FSMContext):
    """Обработка пропуска ввода срока действия (по умолчанию 1825 дней)."""
    admin_ids = get_admin_ids()
    if callback.from_user.id not in admin_ids:
        await callback.answer("Доступ запрещен!", show_alert=True)
        return
    ...
    if result["returncode"] == 0:
        ...
        await callback.message.answer(
            f"✅ Клиент создан!\n📅 Срок действия: 1825 дней (5 лет)",
            reply_markup=create_main_menu(server_ip)
        )
        log_action("bot", callback.from_user.id, callback.from_user.full_name, 
                   "client_create", f"{client_name} (openvpn)")
        await notify_admins(callback.from_user.id, callback.from_user.full_name, 
                           f"создал клиента <b>{client_name}</b> (openvpn)")
    else:
        await callback.message.answer(f"❌ Ошибка: {result['stderr']}")
...
async def send_config(chat_id: int, client_name: str, option: str):
    ...
    try:
        if option == "4":
            name_core = client_name.replace("antizapret-", "").replace("vpn-", "")
            directories = [
                ("/root/antizapret/config/wireguard_amnezia", "AmneziaWG (antizapret)"),
                ("/root/antizapret/config/wireguard_amnezia", "AmneziaWG (vpn)"),
            ]
                ...
                for filename in os.listdir(directory):
                    if pattern.fullmatch(filename):
                        ...
                        if os.path.exists(full_path):
                            ...
                            logger.debug("📁 Найден файл: %s", full_path)
                        break
            except FileNotFoundError:
                logger.warning("⚠️ Директория не найдена: %s", directory)
                continue
        
        for path, config_type in files_found:
            await bot.send_document(chat_id, document=FSInputFile(path), caption=f'🔐 Клиент "{client_name}". {config_type}.')
            logger.debug("📤 Конфиг отправлен пользователю %s: %s", chat_id, os.path.basename(path))
        
        if not files_found:
            logger.warning("⚠️ Файлы конфигураций не найдены для клиента: %s", client_name)
            await bot.send_message(chat_id, "❌ Файлы конфигураций не найдены")
    
    except Exception as e:
        logger.error("❌ Ошибка при отправке конфигурации: %s", e, exc_info=True)
