...
def log_action(source: str, admin_id: str, admin_name: str, action: str, details: str = "", ip_address: str = ""):
    ...
    try:
        with _get_conn() as conn:
            ...
            conn.execute(
                """INSERT INTO admin_log (timestamp, source, admin_id, admin_name, action, details, ip_address)
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (datetime.now().isoformat(), source, str(admin_id), admin_name, action, details, ip_address),
            )
            conn.commit()
        logger.debug("📝 Аудит: %s | %s | %s | %s", source, admin_name, action, details)  # DEBUG вместо INFO
...
async def notify_admins(actor_id: int, actor_name: str, action_text: str):
    ...
    for admin_id in get_admin_ids():
        ...
        try:
            ...
            logger.debug("📬 Уведомление отправлено админу %d", admin_id)
        except Exception as e:
